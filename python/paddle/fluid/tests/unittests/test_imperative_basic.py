        ...
        def test_mlp(sort_sum_gradient):
            ...
            for batch_id in range(24):
                ...
                if ((batch_id + 1) % 8) == 0:
                    ...
                    expected_weight1_grad = np.zeros(
                        mlp2._linear1.weight.grad.shape)
                    expected_bias1_grad = np.zeros(
                        mlp2._linear1.bias.grad.shape)
                    expected_weight2_grad = np.zeros(
                        mlp2._linear2.weight.grad.shape)
                    expected_bias2_grad = np.zeros(
                        mlp2._linear2.bias.grad.shape)
