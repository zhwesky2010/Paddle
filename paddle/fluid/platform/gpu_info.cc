...
std::string GetCudaErrorMessage(cudaError_t e) {
  ...
  if (cudaerror.ByteSizeLong() == 0) {
    ...
    GetModuleFileName(NULL, strModule.c_str(), 64);
    ...
    if (compare_path.compare("avx.pyd") == 0) {
      filePath = strModule + "\\..\\include\\third_party\\cudaerror\\data\\cudaErrorMessage.pb";
    }
    else {
      filePath = strModule + "\\..\\third_party\\cudaerror\\data\\cudaErrorMessage.pb";
    }
#else
    Dl_info info;
    if (dladdr((void*)GetCudaErrorMessage, &info)) {
      ...
      if (compare_path.compare("avx.so") == 0) {
        filePath = strModule + "/../include/third_party/cudaerror/data/cudaErrorMessage.pb";
      }
      else {
        filePath = strModule + "/../third_party/cudaerror/data/cudaErrorMessage.pb";
      }
    }
#endif
      ...
      if (cuda_version == cudaerror.allmessages(i).version()) {
        for (int j = 0; j < cudaerror.allmessages(i).messages_size(); ++j) {
          if (e == cudaerror.allmessages(i).messages(j).errorcode()) {
            sout << "\n\nRecommended Solution: " << cudaerror.allmessages(i).messages(j).errormessage()
                 << " This error occurred ";
            return sout.str();
          }
        }
...
static int GetCUDADeviceCountImpl() {
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      ...
      platform::errors::External("cudaGetDeviceCount failed in "
                                 "paddle::platform::GetCUDADeviceCountImpl. "));
  return count;
}
...
dim3 GetGpuMaxGridDimSize(int id) {
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      ...
      platform::errors::External("cudaDevAttrMaxGridDimY failed in "
                                 "paddle::platform::GetGpuMaxGridDimSize. "));
  ret.y = size;
...
int GetCUDARuntimeVersion(int id) {
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      ...
      platform::errors::External("cudaRuntimeGetVersion failed in "
                                 "paddle::platform::GetCUDARuntimeVersion. "));
...
int GetCUDAMultiProcessors(int id) {
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      ...
      platform::errors::External("cudaDeviceGetAttribute failed in "
                                 "paddle::platform::GetCUDAMultiProcess. "));
  return count;
}
...
int GetCUDAMaxThreadsPerBlock(int id) {
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      error_code, platform::errors::External(
                      "cudaDeviceGetAttribute failed in "
                      "paddle::platform::GetCUDAMaxThreadsPerBlock. "));
  return count;
}
...
void GpuMemcpyPeerAsync(void *dst, int dst_device, const void *src,
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      ...
      platform::errors::External("cudaMemcpyPeerAsync failed in "
                                 "paddle::platform::GpuMemcpyPeerAsync. "));
}
...
void GpuMemcpyPeerSync(void *dst, int dst_device, const void *src,
  ...
  PADDLE_ENFORCE_CUDA_SUCCESS(
      ...
      platform::errors::External("cudaMemcpyPeer failed in "
                                 "paddle::platform::GpuMemcpyPeerSync. "));
}
